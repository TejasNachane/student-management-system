...
if (!isLoggedIn() || getUserType() !== 'faculty' || (isset($_SESSION['role']) && $_SESSION['role'] !== 'principal')) {
    showNotification('Only principals can add new courses', 'error');
        ...
        try {
            ...
            if ($check_stmt->fetch()) {
                ...
                if ($insert_stmt->execute([$course_code, $course_name, $description, $credits, $semester, $department])) {
                    ...
                    showNotification('Course "' . $course_name . '" added successfully', 'success');
...
try {
    $database = new Database();
    $db = $database->getConnection();
    $dept_query = "SELECT DISTINCT department FROM courses WHERE department IS NOT NULL AND department != '' ORDER BY department";
...
<body>
    ...
    <div class="container">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="course_code" name="course_code" class="form-control" 
                               ...
                               required placeholder="e.g., CS101, MATH201" style="text-transform: uppercase;">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="department" name="department" class="form-control" 
                               ...
                               placeholder="e.g., Computer Science, Mathematics" list="departmentList">
        ...
        <div class="card" style="background: linear-gradient(135deg, #9b59b6, #8e44ad); color: white;">
            <div class="card-header" style="border-bottom: 1px solid rgba(255,255,255,0.2);">
                <h3 style="margin: 0; color: white;">Quick Course Templates</h3>
            </div>
            <div style="padding-top: 1rem;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                    <button type="button" class="btn btn-light" onclick="fillTemplate('CS101', 'Introduction to Computer Science', 'Basic concepts of programming and computer science fundamentals', 3, 1, 'Computer Science')">CS Course</button>
                    <button type="button" class="btn btn-light" onclick="fillTemplate('MATH101', 'Calculus I', 'Differential and integral calculus with applications', 4, 1, 'Mathematics')">Math Course</button>
                    <button type="button" class="btn btn-light" onclick="fillTemplate('ENG101', 'English Composition', 'Academic writing and communication skills', 3, 1, 'English')">English Course</button>
                    <button type="button" class="btn btn-light" onclick="fillTemplate('PHY101', 'General Physics', 'Mechanics, heat, and thermodynamics', 4, 1, 'Physics')">Physics Course</button>
                </div>
            </div>
        </div>
        ...
        <div class="card" style="background: linear-gradient(135deg, #3498db, #2980b9); color: white;">
            ...
            <div style="padding-top: 1rem;">
                <ul style="margin: 0; padding-left: 1.5rem;">
                    ...
                    <li>Group courses by department for better organization</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function fillTemplate(code, name, description, credits, semester, department) {
        ...
        // Auto-generate course code from department and course name
        function generateCourseCode() {
            ...
            if (department && courseName) {
                let deptCode = '';
                if (department.toLowerCase().includes('computer')) deptCode = 'CS';
                else if (department.toLowerCase().includes('math')) deptCode = 'MATH';
                else if (department.toLowerCase().includes('english')) deptCode = 'ENG';
                else if (department.toLowerCase().includes('physics')) deptCode = 'PHY';
                else if (department.toLowerCase().includes('chemistry')) deptCode = 'CHEM';
                else if (department.toLowerCase().includes('biology')) deptCode = 'BIO';
                else deptCode = department.substring(0, 3).toUpperCase();
                
                const number = '101'; // Default starting number
                const codeField = document.getElementById('course_code');
                if (codeField.value === '') {
                    codeField.value = deptCode + number;
                }
            }
        }

        // Attach event listeners
        document.getElementById('department').addEventListener('blur', generateCourseCode);
        document.getElementById('course_name').addEventListener('blur', generateCourseCode);
    </script>
